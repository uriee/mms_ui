...
class Permissions extends PureComponent {
  ...
  calcChecked= (routes,profile) => {
    var defaultChecked =  routes.map((item,i) => {
      if (item.routes && item.routes.length) {

        const parent = item.routes.reduce((o,x)=> o && x.authority && x.authority.includes(profile) ,true)
  ...
  sAuth = (authorityArr,profile,checked) => (
            (authorityArr && authorityArr.length ? 
              (authorityArr.includes(profile) ? 
                (checked ? authorityArr : authorityArr.filter(x=>x !== profile)) :
                  (checked ? [...authorityArr,profile] : authorityArr)) :
                    (checked ? [profile] : []))
  )
  ...
  setAuth = (routes,profile) => routes
    .map((item,i) => {
      ...
      if (item.routes && item.routes.length) {
        ...
        item.authority = this.sAuth(item.authority,profile,this.checked.includes(item.name+"_pkey"+i))     
        return item
    }); 
...
render() {
  ...
  const renderTree = (routes) => routes
    .map((item,i) => {
              /*console.log("___3___",item,i)*/
      if (item.routes && item.routes.length) {
        return <TreeNode key={item.name+"_pkey"+i} title={ formatMessage({ id: `menu${item.path.replace(/\/|_/g,'.')}` }) }>{renderTree(item.routes)}</TreeNode> 
      }
        return <TreeNode key={item.name+"_pkey"+i} title={ formatMessage({ id: `menu${item.path.replace(/\/|_/g,'.')}` }) } /> 
    });   
      ...
      <Form onSubmit={this.handleSubmit}>
          ...
          getFieldDecorator('profile')(
            ...
            >
              {this.props.permissions.list &&
               this.props.permissions.list.
                map(profile =><Option key={profile.name} value={profile.name}>{profile.name}</Option>)}
            </Select>
          )}
        </Form.Item>   
        ...
        >{
          getFieldDecorator('type')(
            <Select
              placeholder="Edit Access Or Permissions"
